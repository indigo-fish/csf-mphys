...
def graph_one(axes, dataset='CSF-20C', season='DJF', variance=True, trend=True, cut_years=False):
	...
	if dataset=='CSF-20C': colour='black'
	elif dataset=='ASF-20C': colour='blue'
	elif dataset=='SEAS5': colour='green'
	l1 = axes.plot(times, mean_SAM_indices, linewidth = 2, color = colour)
	l2 = axes.plot(years_SAM, Marshall_SAM_index, linewidth = 2, color = 'gray')
	...
	if trend:
		...
		#plots trend lines
		axes.plot(times, csf_line, color=colour, linestyle='dashed')
		axes.plot(years_SAM, Marshall_line, color='gray', linestyle='dashed')
...
def graph_all(season='DJF', variance=True, trend=True, cut_years=False):
	...
	for dataset, axis in zip(datasets, axes):
		l1, l2 = graph_one(axis, dataset=dataset, season=season, variance=variance, trend=trend, cut_years=cut_years)
		if dataset != 'SEAS5':
			lines.append(l1)
			lines.append(l2)
		else: lines.append(l1)
	labels = ['CSF-20C', 'ASF-20C', 'SEAS5', 'Marshall']
	fig.legend([l1, l2], labels=labels, loc='center right')
	title = 'SAM Indices in ' + season
	fig.suptitle(title)
	fig.tight_layout()
	
	figure_name = Figure_dir + 'Modified_Multiseries_' + season
