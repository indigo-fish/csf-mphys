...
def runregression(variable, season, level_to_choose=None, dataset=None, shift_years=False):
    ...
    if variable == 'zg':
        var2,_,_,_,times2,_,_ = reading_in_data_functions.read_in_variable(file_str_ERA5_2,'geopotential_height',chosen_level=level_to_choose) # weirdly, these two files use different variable names zg vs geopotential_height
    else:
        var2,_,_,_,times2,_,_ = reading_in_data_functions.read_in_variable(file_str_ERA5_2,variable,chosen_level=level_to_choose)
    ...
    if variable=='tos': clevs = np.arange(-.5, .6, .1)
    else: clevs = np.linspace(-20, 15, 10)
    ...
    # save figure
    figure_name = Figure_dir + 'SAM_pattern_' + variable + str(level_to_choose)
    if dataset!=None: figure_name += '_' + dataset
    if shift_years: figure_name += '_offset1'
    figure_name += '_ERA5_' + str(start_year) + '-' + str(end_year) + '_' + season + '.png'
...
for season in seasons:
    for dataset in datasets:
        runregression('tos', season, dataset=dataset, shift_years=False)
        runregression('tos', season, dataset=dataset, shift_years=True)
"""

season = 'DJF'
runregression('zg', season, level_to_choose = 500)
