...
import era_for_persistence as era_perst
...
def persistence(axes, dataset='CSF-20C', season='DJF', compare_SEAS5 = True):
	...
	Marshall_SAM_indices, Marshall_years = era_perst.get_era_SAM_indices(season)
	...
	axes.plot(masked_Marshall, masked_SAM, 'o', label='original data')
	
	res = linregress(masked_Marshall, masked_SAM)
	xpts = np.arange(-1.5, 1.9, 0.1)
	lineplot = []
	for point in xpts:
		lineplot.append(point * res.slope + res.intercept)
	
	axes.plot(xpts, lineplot, 'r', label='fitted line')
	axes.set(xlabel='ERA5 SAM', ylabel= dataset + ' SAM')
	significance = sig_test.significance(masked_Marshall, masked_SAM, 1)
